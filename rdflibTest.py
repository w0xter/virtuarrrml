...
class MappingSimplifier:
    ...
    def __MinGraph_to_ttl(self):
        self.minGraph.serialize(format="ttl", destination="tmp/min.mapping.ttl")


def main():
    dirPath = "test/" + ["gtfs","motivationexample"][1]
    query =  "query4"
    queryPath = "%s/%s/"%(dirPath,query)
    sparql  = Sparql(queryPath + "query.rq" )
    rdflibUtils = MappingSimplifier(sparql.uris, mapping=dirPath+"/mapping.ttl")
